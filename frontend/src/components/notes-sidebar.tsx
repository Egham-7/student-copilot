...
export function NotesSidebar() {
  ...
  return (
    <Sidebar className="bg-background border-r border-border">
      <SidebarHeader>
        <h2 className="text-title px-2 py-2 text-sm font-semibold">My Notes</h2>
      </SidebarHeader>
            ...
            {notes?.map((note) => {
              ...
              return (
                <SidebarMenuItem key={note.id}>
                  <Link to="/$noteId" params={{ noteId: note.id.toString() }}>
                  ...
                  <SidebarMenuAction onClick={() => handleDeleteNote(note.id)}>
